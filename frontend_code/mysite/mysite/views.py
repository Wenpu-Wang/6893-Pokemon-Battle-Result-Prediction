...
def getPredictions(name1, name2):
    ...
    for i in range(6):
        ...
        pokemon_info = json.loads(response.text)
        data[i] = pokemon_info['stats'][i]['base_stat']

    for i in range(6):
        response = requests.get(url.format(name2))
        pokemon_info = json.loads(response.text)
        data[i+6] = pokemon_info['stats'][i]['base_stat']
...
def result1(request):
    ...
    url = get_pic(name)
    if res[1] == 'none':
        return render(request, 'refresh.html', {'first_url':url,'first_poke_name': name})
    else:
        return render(request, 'refresh2.html', {'first_url':get_pic(res[0]), 'second_url':get_pic(res[1]),'first_poke_name': res[0],'second_poke_name':res[1]})
...
def result2(request):
    ...
    url2 = get_pic(res[1])
    if res[0] == 'none':
        return render(request, 'refresh1.html', {'second_url':url2,'second_poke_name': name})
    #result = getPredictions(pclass, sex, age, sibsp, parch, fare, embC, embQ, embS)
    else:
        return render(request, 'refresh2.html', {'first_url':get_pic(res[0]), 'second_url':url2,'first_poke_name': res[0],'second_poke_name':res[1]})
...
def result(request):
    ...
    return render(request, 'result.html', {'first_url':url1, 'second_url':url2,'first_poke_name': name1,'second_poke_name':name2,'winner':winner})
